...
| **멀티 테크노킹** | `setup-team` 이 **기본 3-pane** 킹 윈도우 부트스트랩(pane 0 메인 + 로드 시 `/workflows:handoff --resume` 자동, 1·2 추가 킹). `CLAUDE_TEAM_KINGS` 조절·**무제한 확장**. atomic 카운터 + `merge-gate.sh` 직렬화로 충돌 0 | G6 |
