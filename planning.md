...
- **추론 effort**: **전 패널·서브에이전트 = xhigh 통일**. 메인 Technoking도 xhigh — 비차단성은 effort가 아니라 *구조*(모든 실작업을 백그라운드로 던짐)로 보장되므로 패널 effort를 낮출 이유가 없음. 서브에이전트는 패널 effort(xhigh)를 상속(per-subagent effort 인자는 Agent/Workflow 툴에 없음). `config.yml.effort`(technoking·subagent 모두 xhigh).
...
- 스택: 자체 구현(Next.js 16 + SSE 라이브 tail, MVP). `/status` CLI 보드는 폴백으로 유지.
- **기동**: `setup-team` 이 백그라운드로 자동 실행 + URL 안내(`DASHBOARD_DIR`/`DASH_PORT`, `EVENTS_LOG`=프로젝트 events.jsonl).
