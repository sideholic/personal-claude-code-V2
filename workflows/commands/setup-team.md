...
3. **tmux `claude-team` 세션(독립)** — Technoking pane 0/1/2 (세로 33%씩). pane 0 = 메인(welcome 배너), pane 1·2 = 추가 킹. 개수는 `CLAUDE_TEAM_KINGS`(기본 3). tmux 안에서 실행 시 새 세션으로 자동 전환(`switch-client`), 밖이면 `tmux attach -t claude-team`.

스크립트 실행 후 사용자에게 **대시보드 URL · tmux attach 안내(새 세션일 때)**를 보고한다. 리뷰어는 독립 리뷰어 서브에이전트(Claude Opus, `effort: 'max'`)라 별도 준비 상태 점검이 불필요 — 외부 준비 게이트 없음. tmux 미설치 시 pane 부트스트랩만 생략하고 나머지는 진행.
