...
4. **build** [BG, parallel] — per non-overlapping unit, one worktree + lane (claim transitions queued→in_progress via `bin/ticket-transition.sh`, keeping board + events in sync): `qa-pre`(fail-first RED) → impl(`backend`/`frontend`) → `review`(independent reviewer subagent — general for small·medium, adversarial for large; awaited in-lane) + in-lane rescue. No stops.
...
## Dispatch (G0 / G5)
- Skills/subagents run on **opus** at **xhigh** effort by default (the king launches at xhigh and background subagents inherit it, `config.yml.effort`); the **reviewer subagent runs at `effort: 'max'`** (still opus) so it reviews deeper than the implementer. Per-agent effort is honored **only** via the Workflow `agent()` primitive (the Agent tool has no `effort` param) — so the review step always runs inside a Workflow lane. Non-blocking is structural (work → background), not from a lower king effort.
...
- Heavy dynamic `Workflow` **only when ≥2 non-overlapping units**. 1 unit or `/task` = single inline BG subagent (no Workflow ceremony) — but its review step is still spawned via `agent({effort:'max'})` inside a lane.
- The one synchronous await (the reviewer subagent) lives INSIDE a lane; the king launched that lane in the background, so the king never blocks.

## Escalation (always surface, any tier)
`requirements_change` · `architectural_change` · `untestable_ac` · review 3× BLOCKING · rescue validation fail · `reviewer_unavailable` (reviewer subagent fails after a retry — only that lane pauses; main convo + other lanes continue; never merge without a completed review). **Auto-rescue** (`error_2x` / `pattern_stuck`) fires WITHOUT user approval, ≤1 per ticket per signature, never rescue-of-a-rescue.
...
## /task (small shortcut)
classify → single inline lane (qa-pre → impl → reviewer subagent [general — small] + rescue) → merge. 0 stops. Review/rescue/merge gates still fire.
