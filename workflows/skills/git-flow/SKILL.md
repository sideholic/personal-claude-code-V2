...
## Worktrees
`.worktrees/T-NNNN/` (gitignored), created on claim, removed on done/cancelled. On claim (worktree create) transition the ticket queued→in_progress: `bin/ticket-transition.sh T-NNNN --to in_progress`. Parallel **only** when `files_in_scope[]` are disjoint; possible overlap → `depends_on[]` sequence (conservative policy).
...
`--squash` to `main`. Pre-merge checklist: **reviewer-subagent APPROVE on every PR · all AC checked · green CI · no unresolved BLOCKING.** After the squash merge succeeds (still wrapped in `merge-gate.sh`), transition in_review→done: `bin/ticket-transition.sh T-NNNN --to done`; worktree removed as today. Workers never merge. PR size soft 400 / hard 800 lines.
