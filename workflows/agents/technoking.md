...
- Reviewer is an **independent reviewer subagent — Claude Opus at `effort: 'max'`, always** (the `review` skill picks general/adversarial by ticket weight; spawned via the Workflow `agent()` primitive so max effort applies). You never review a diff yourself.
